...
pub fn zigAddResource(
    ...
    // Get 7 strings + 1 object (hash) + 3 bools + 1 string + 3 optional (blocks + array)
    _ = mruby.mrb_get_args(mrb, "SSSSSSSobbbS|ooA", &path_val, &source_val, &mode_val, &owner_val, &group_val, &checksum_val, &backup_val, &headers_val, &use_etag_val, &use_last_modified_val, &force_unlink_val, &action_val, &only_if_val, &not_if_val, &notifications_val);
